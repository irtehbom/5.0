...
_unit spawn
{
	...
	waitUntil {
		_Timer ctrlSetText format[localize "STR_Medic_Respawn",[(_maxTime + life_medic_requested - time),"MM:SS.MS"] call BIS_fnc_secondsToString];
		round(_maxTime + life_medic_requested - time) <= 0 OR isNull _this
	};
...
if(!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
	...
	[[player, _killer],"life_fnc_checkTarget",false,false] spawn life_fnc_MP;
...
if(side _killer == west && playerSide != west) then {
	...
	if(!life_use_atm && {ftp_cash > 0}) then {
		[format[localize "STR_Cop_RobberDead",[ftp_cash] call life_fnc_numberText],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
		ftp_cash = 0;
	};
};
