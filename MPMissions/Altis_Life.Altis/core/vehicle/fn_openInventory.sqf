...
if (isNull _vehicle || !(_vehicle isKindOf "Car" || _vehicle isKindOf "Air" || _vehicle isKindOf "Ship" || _vehicle isKindOf "Box_IND_Grenades_F" || _vehicle isKindOf "B_supplyCrate_F" || _vehicle isKindOF "Box" || _vehicle isKindOf "House_F" ||  _vehicle isKindOf "Box_IND_AmmoVeh_F")) exitWith {}; //Either a null or invalid vehicle type.
...
if (_vehicle isKindOf "Box_IND_Grenades_F" || _vehicle isKindOf "B_supplyCrate_F" ||  _vehicle isKindOf "Box_IND_AmmoVeh_F") then {
...
if(_vehicle isKindOf "House_F") then {
	if(((_vehicle getVariable "estate_owned") select 0) == (getPlayerUID player)) then {
	ctrlSetText[3501,"Real Estate Storage"];
    } else {
	ctrlSetText[3501,format[(localize "STR_MISC_HouseStorage")+ " - %1",getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName")]];
	};
} else {
...
_vehicle spawn {
	...
	if(((_this getVariable "estate_owned") select 0) == (getPlayerUID player)) then {
			[_this] remoteExecCall ["TON_fnc_updateYinv",2];
	};
};
