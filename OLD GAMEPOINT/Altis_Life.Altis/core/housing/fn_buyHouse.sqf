...
if(_action) then {
	if(ftp_atmcash < (_houseCfg select 0)) exitWith {hint format [localize "STR_House_NotEnough"]};
	...
	ftp_atmcash = ftp_atmcash - (_houseCfg select 0);
	...
	[[_uid,_house,profileName],"TON_fnc_addHouse",false,false] spawn life_fnc_MP;
};