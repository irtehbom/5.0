...
if(_curTarget isKindOf "House_F" && {player distance _curTarget < 12} OR ((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curTarget) OR (nearestObject [[22919,17016.6,0.00143886],"Land_Dome_Big_F"])  == _curTarget OR (nearestObject [[24525.4,19186.7,16.5238],"Land_Research_HQ_F"]) == _curTarget)  exitWith {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	if ((_curTarget getVariable["restrained", false] || _curTarget getVariable["hostage", FALSE]) && !dialog) then {
		switch (playerSide) do
		{
			...
			case east:
			{
				[_curTarget] call life_fnc_bountyInteractionMenu;
			};
		};
	};
