...
if (count _houseCfg isEqualTo 0 && playerSide isEqualTo east) exitWith {};
...
if (_curTarget isKindOf "House_F" && playerSide isEqualTo west) exitWith {
    ...
    if ((nearestObject [_pos,"Land_Dome_Big_F"]) isEqualTo _curTarget || (nearestObject [_pos,_vaultHouse]) isEqualTo _curTarget || _oilRigBuilding isEqualTo _curTarget) then {
