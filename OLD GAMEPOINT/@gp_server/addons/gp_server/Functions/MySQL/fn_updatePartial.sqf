...
switch(_mode) do {
	...
	case 8: {
		_pl = [_this, 2, ObjNull, [ObjNull]] call BIS_fnc_param;
		_query = format["UPDATE players SET experience='%1', level='%2', skillpoints='%3', skills='%4' WHERE playerid='%5'", _pl getVariable "Experience", _pl getVariable "Level", _pl getVariable "SkillPoints", format["%1", _pl getVariable "Skills"], _uid];
	};
};
