...
if (_existingItem != -1) then {
	...
	// Debug stuff:
	//diag_log format["CONFIG: %1 TYPE: %2",_weaponConfigEntry, _type];
	//diag_log format["MODEL: %1 SCOPE: %2",_model, _scope];

	if ( _model != "" ) then {
		
		if ( _model splitString "\" select 1  splitString "_" select 0 == "Weapons") then {
		
			_rightList lbAdd format["%1", _weaponName];
			//diag_log format["%1 and item %2", _data, _item];
