...
_RentButton = _display displayCtrl RentButton;
_Btn1 ctrlShow false;
_Btn2 ctrlShow false;
_Btn3 ctrlShow false;
_Btn4 ctrlShow false;
_RentButton ctrlShow false;

life_pInact_curTarget = _curTarget;
if(_curTarget isKindOf "House_F" && playerSide == west) exitWith {
	if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"])       == _curTarget OR
	(nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curTarget OR
    (nearestObject [[24525.4,19186.7,16.5238],"Land_Research_HQ_F"]) == _curTarget OR
	(nearestObject [[22942.2,17015.1,0.00143886],"Land_Dome_Big_F"])   == _curTarget) then {
...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget getVariable "house_owner"}) then {
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		_RentButton buttonSetAction "[life_pInact_curTarget] spawn life_fnc_housingPayRent; closeDialog 0;";
		_RentButton ctrlSetText "Pay Rent";
		_RentButton ctrlShow true;
	};
};