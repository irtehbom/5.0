...
// Right, playing a car alarm is not simply say3D and be done with it. If you cancel the lockpick, and start again, it will play twice through eachother.
// So let's not just do this: _curTarget say3D "caralarm";

// First check if any attached objects are there.
{
	deleteVehicle _x; // Stops any currently playing sounds.
} forEach (attachedObjects _curTarget);
// Then create a helipad.
_helipad = "Land_HelipadEmpty_F" createVehicle [0,0,0];
_helipad attachTo[_curTarget, [0,0,0]];
_helipad say3D "caralarm";
