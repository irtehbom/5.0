...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if((count crew _vehicle) == 0) then
	{
		...
		_disablepolice = _vehicle getVariable["disable_police_impound",''];
	    if (!(_disablepolice == "cop")) then 
    	{
		[[9, player, (call life_donator)], "life_fnc_xpGenericXPGain", false, false] spawn life_fnc_MP;
	    };
