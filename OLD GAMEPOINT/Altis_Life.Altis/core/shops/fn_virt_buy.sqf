...
if((_price * _amount) > ftp_cash && {!isNil "_hideout" && {!isNil {grpPlayer getVariable "gangBank"}} && {(grpPlayer getVariable "gangBank") < _price * _amount}}) exitWith {hint localize "STR_NOTF_NotEnoughMoney"};
...
if(([true,_type,_amount] call life_fnc_handleInv)) then
{
	...
	if(!isNil "_hideout" && _allowed && {!isNil {grpPlayer getVariable "gangBank"}} && {(grpPlayer getVariable "gangBank") >= (_price * _amount)}) then {
		_action = [
			format[(localize "STR_Shop_Virt_Gang_FundsMSG")+ "<br/><br/>" +(localize "STR_Shop_Virt_Gang_Funds")+ " <t color='#8cff9b'>$%1</t><br/>" +(localize "STR_Shop_Virt_YourFunds")+ " <t color='#8cff9b'>$%2</t>",
				[(grpPlayer getVariable "gangBank")] call life_fnc_numberText,
				[ftp_cash] call life_fnc_numberText
			],
		...
		if(_action) then {
			...
			if((_price * _amount) > ftp_cash) exitWith {[false,_type,_amount] call life_fnc_handleInv; hint localize "STR_NOTF_NotEnoughMoney";};
			hint format[localize "STR_Shop_Virt_BoughtItem",_amount,_name,[(_price * _amount)] call life_fnc_numberText];
			__SUB__(ftp_cash,_price * _amount);
			[[1,player,life_shop_type,_amount,_price,_type],"TON_fnc_adjustPrices",false,false] spawn life_fnc_MP;
		};
	} else {
		if (_allowed && {!isNil {grpPlayer getVariable "gangBank"}} && {(grpPlayer getVariable "gangBank") >= _price}) then {
			_action = [
			format[(localize "STR_Shop_Virt_Gang_FundsMSG")+ "<br/><br/>" +(localize "STR_Shop_Virt_Gang_Funds")+ " <t color='#8cff9b'>$%1</t><br/>" +(localize "STR_Shop_Virt_YourFunds")+ " <t color='#8cff9b'>$%2</t>",
			[(grpPlayer getVariable "gangBank")] call life_fnc_numberText,
			[ftp_cash] call life_fnc_numberText
