...
if ((_oilRigBuilding == _building) && (west countSide playableUnits < (LIFE_SETTINGS(getNumber,"minimum_cops")))) exitWith {
    hint format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"minimum_cops"))];
};

if (isNil "life_boltcutter_uses") then {life_steel_bolt_cutter_uses = 0;};
...
if (_oilRigBuilding == _building) then {
	[_oilRigBuilding,"bank_alarm"] remoteExec ["life_fnc_say3D",RANY];
    [[1,2],"STR_ISTR_Alert_Oilrig",true,[]] remoteExecCall ["life_fnc_broadcast",RCLIENT];
};
...
switch (typeOf _building) do {
    case _oilRigBuildingType: {_cpRate = 0.003;};
    default {}
};
...
life_steel_bolt_cutter_uses = life_steel_bolt_cutter_uses + 1;
life_action_inUse = false;

if (life_steel_bolt_cutter_uses >= 5) then {
    [false,"boltcutter",1] call life_fnc_handleInv;
    life_steel_bolt_cutter_uses = 0;
};
