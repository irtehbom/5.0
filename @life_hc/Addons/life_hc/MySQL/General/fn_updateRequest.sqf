...
_side = [_this,2,sideUnknown,[east]] call BIS_fnc_param;
...
_position = if (_side isEqualTo east) then {[_position] call HC_fnc_mresArray} else {[]};
...
switch (_side) do {
    ...
    case east: {_query = format ["UPDATE players SET name='%1', cash='%2', bankacc='%3', civ_licenses='%4', civ_gear='%5', arrested='%6', civ_stats='%7', civ_alive='%8', civ_position='%9', playtime='%10' WHERE pid='%11'",_name,_cash,_bank,_licenses,_gear,[_this select 8] call HC_fnc_bool,_stats,[_alive] call HC_fnc_bool,_position,_playtime_update,_uid];};
