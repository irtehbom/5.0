...
if (!isNull GP_HC) then {
	...
	[_params, _code, owner GP_HC, false] spawn life_fnc_MP;
} else {
	diag_log format["[HC] Execing '%1' on server", _code];
	_params spawn (missionNamespace getVariable[_code, { diag_log "[HC] Execing on server failed - function not found in missionNamespace"; }]);
};
