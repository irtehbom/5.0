...
if(_action) then {
	if(ftp_atmcash < (_real_estate_prices select 0)) exitWith {hint format ["You don't have enough money to purchase this."]};
	...
	ftp_atmcash = ftp_atmcash - (_real_estate_prices select 0);
	...
	[[_uid,_house,profileName],"TON_fnc_insertRealEstate",false,false] spawn life_fnc_MP;
};