...
if(playerSide == west && _illegal != -1) exitWith
{
	...
	ftp_atmcash = ftp_atmcash + ((life_illegal_items select _illegal) select 1);
	deleteVehicle _obj;
...
_skillRank = [player, 107] call life_fnc_xpSkillRank; // Bodybuilder
if (_skillRank > 0) then {
	_maxWeight = life_maxWeight + (_skillRank * 10);
};

_diff = [_itemInfo select 0,_itemInfo select 1,life_carryWeight,_maxWeight] call life_fnc_calWeightDiff;
