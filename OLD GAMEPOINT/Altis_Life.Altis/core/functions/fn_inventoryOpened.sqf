...
if ((typeOf _container) in life_build_storage_crates) then {
	_gangId = (group player) getVariable["gangId", -1];
	_objectGangId = _container getVariable["gang", -2];
	
	if (_gangId != _objectGangId) then {
		hint "Only gang members are able to access gang storage.";
		[] spawn {
			waitUntil {!isNull (findDisplay 602)};
			closeDialog 0;
		};
...
if ((typeOf _container) in real_estate_storage) then {
	...
	_estateLock = _container getVariable["estate_locked",true];
	
	if (_estateLock) then {
		hint "This crate is locked, open your estate menu to unlock it";
		[] spawn {
			waitUntil {!isNull (findDisplay 602)};
			closeDialog 0;
		};
