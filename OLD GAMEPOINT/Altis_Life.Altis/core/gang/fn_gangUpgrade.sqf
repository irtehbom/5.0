...
if (_level == life_gang_plot_rank) then {
	_plotPole = true;
	_message = _message + " This upgrade includes a plotpole to build your base! Make sure you have atlcivilian one virtual inventory slot available.";
};
...
if (_action) then {
	...
	[[10, (group player)], "life_fnc_gangUpdate", false, false] call life_fnc_MP;
