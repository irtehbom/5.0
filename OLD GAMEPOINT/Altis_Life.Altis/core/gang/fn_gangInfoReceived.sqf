...
if (isNil "_group" || isNull _group) then {
	...
	_group setVariable["gangName", _gangInfo select 3, true];
	_group setVariable["gangLeader", _gangInfo select 1, true];
	_group setVariable["gangLevel", _gangInfo select 2, true];
	_group setVariable["gangBank", _gangInfo select 4, true];
};
...
if (getPlayerUID player == _gangInfo select 1) then {
	...
	[[player, _group], "life_fnc_gangLeader", units _group, false] spawn life_fnc_MP;
};

[[_group, player], "life_fnc_buildGetActionObjects", false, false] spawn life_fnc_MP;